...
const state = {
  ...
  rakutenManualMailPage: 1,
  rakutenManualMailPageSize: 30,
...
function renderRakutenMailConnectionFilter() {
  const scopedConnectionId = String(state.rakutenMailScopedConnectionId || "");
  ...
  if (filter) {
    ...
    filter.value = scopedConnectionId || selected;
    filter.disabled = Boolean(scopedConnectionId);
  }
  ...
  if (runFilter) {
    ...
    runFilter.value = scopedConnectionId || selected;
    runFilter.disabled = Boolean(scopedConnectionId);
  }
  ...
  if (templateSelect) {
    ...
    templateSelect.value = scopedConnectionId || selected || String(state.rakutenRmsConnections[0]?.id || "");
    templateSelect.disabled = Boolean(scopedConnectionId);
  }
  const scopedConnection = state.rakutenRmsConnections.find(
    (connection) => String(connection?.id || "") === scopedConnectionId,
  );
  setTextById(
...
function renderRakutenAutomationHealth() {
  ...
  if (state.rakutenAutomationHealthLoading) {
    meta.textContent = "正在检查当前店铺的配置和任务积压情况...";
    body.innerHTML = '<tr><td colspan="9" class="muted">正在读取健康状态...</td></tr>';
    return;
  }
  const health = state.rakutenAutomationHealth || {};
  const allRows = Array.isArray(health.items) ? health.items : [];
  const scopedConnectionId = String(state.rakutenMailScopedConnectionId || "");
  const rows = scopedConnectionId
    ? allRows.filter((item) => String(item?.connectionId || "") === scopedConnectionId)
    : allRows;
  const summary = rows.reduce((counts, item) => {
...
function rakutenManualAutomationKey(item) {
...
function renderRakutenManualMailStage(item, emptyText) {
  ...
  const recoveryActions = status === "uncertain"
    ? `<button type="button" class="tiny-btn" data-action="markRakutenManualMailSent" data-id="${escapeHtml(item.id)}">确认已发送</button><button type="button" class="tiny-btn" data-action="retryRakutenManualMail" data-id="${escapeHtml(item.id)}">确认重发</button>`
    : status === "dead_letter"
      ? `<button type="button" class="tiny-btn" data-action="retryRakutenManualMail" data-id="${escapeHtml(item.id)}">处理后重试</button>`
      : "";
...
function renderRakutenManualAutomation() {
  ...
  if (selectAll) {
    selectAll.disabled = state.rakutenManualAutomationLoading || executableItems.length === 0;
    selectAll.checked = executableItems.length > 0 && selectedCount === executableItems.length;
    selectAll.indeterminate = selectedCount > 0 && selectedCount < executableItems.length;
  }
  ...
  $("rakutenManualMailFilters")?.classList.toggle("hidden", isShipping);
  $("rakutenManualMailPager")?.classList.toggle("hidden", isShipping);
  if (!isShipping) {
    setTextById(
      "rakutenManualMailPageMeta",
      `第 ${state.rakutenManualMailPage} / ${Math.max(1, state.rakutenManualMailTotalPages)} 页，共 ${state.rakutenManualMailTotalOrders} 个待处理订单`,
    );
    const prev = $("prevRakutenManualMailPageBtn");
    const next = $("nextRakutenManualMailPageBtn");
    if (prev) prev.disabled = state.rakutenManualAutomationLoading || state.rakutenManualMailPage <= 1;
    if (next) next.disabled = state.rakutenManualAutomationLoading || state.rakutenManualMailPage >= state.rakutenManualMailTotalPages;
  }
  ...
  if (summary) {
    const orderCount = isShipping
      ? new Set(items.map((item) => `${String(item?.connectionId || "")}:${String(item?.orderId || "")}`)).size
      : state.rakutenManualMailTotalOrders;
  ...
  body.innerHTML = items.map((item) => {
    ...
    const kindLabel = item?.kind === "shipping" ? "单号回传" : RAKUTEN_MAIL_EVENT_LABELS[item?.event] || "发送邮件";
    const status = String(item?.status || "");
    const note = String(item?.blockedReason || item?.lastError || "").trim();
    return `<tr class="${executable ? "" : "is-disabled"}">
      <td><input type="checkbox" data-action="selectRakutenManualAutomation" data-key="${escapeHtml(key)}" ${selectedKeys.has(key) ? "checked" : ""} ${executable ? "" : "disabled"} /></td>
...
async function loadRakutenManualAutomationActions(kind = state.rakutenManualAutomationKind) {
  ...
  try {
    const mailQuery = requestedKind === "mail" ? {
      connectionId: String($("rakutenManualMailConnectionFilter")?.value || ""),
      orderId: String($("rakutenManualMailOrderFilter")?.value || "").trim(),
      fulfillmentType: String($("rakutenManualMailFulfillmentFilter")?.value || ""),
      status: String($("rakutenManualMailStatusFilter")?.value || ""),
      page: state.rakutenManualMailPage,
      pageSize: state.rakutenManualMailPageSize,
    } : {};
    const result = await request("/rakuten-rms-api/automation/manual-actions/preview", {
      method: "POST",
      body: JSON.stringify({ kind: requestedKind, ...mailQuery }),
    }) || {};
    ...
    if (requestedKind === "mail") {
      state.rakutenManualMailPage = Number(result.page || 1);
      state.rakutenManualMailTotalOrders = Number(result.totalOrders || 0);
      state.rakutenManualMailTotalPages = Number(result.totalPages || 0);
    }
    setTextById(
...
async function openRakutenManualAutomationWorklist(kind, button) {
  ...
  const isShipping = state.rakutenManualAutomationKind === "shipping";
  if (!isShipping) {
    state.rakutenManualMailPage = 1;
    const select = $("rakutenManualMailConnectionFilter");
    if (select) {
      const selected = select.value;
      select.innerHTML = '<option value="">全部店铺</option>' + state.rakutenRmsConnections
        .filter((connection) => connection?.status === 1 && connection?.mailNotificationsEnabled === true)
        .map((connection) => `<option value="${escapeHtml(connection.id)}">${escapeHtml(connection?.shop?.name || "-")}</option>`)
        .join("");
  ...
  setTextById(
    ...
    isShipping
      ...
      : "仅适用于日本时间2026年9月1日起导入WMS的订单。这里只显示尚未全部发送完成的订单；新規邮件仅针对API新建订单，认领CSV不会重复生成。打开清单不会向买家发送邮件。",
  );
  openModal("rakutenManualAutomationModal");
  await withBusyButton(button, "整理中...", () => loadRakutenManualAutomationActions(requestedKind));
}

async function previewRakutenManualMailAction(item, button) {
  const preview = await withBusyButton(button, "读取中...", () => request(
...
async function executeRakutenManualAutomationItems(items, button, options = {}) {
  ...
  const isShipping = state.rakutenManualAutomationKind === "shipping";
  const shopSummary = !isShipping
    ? Array.from(items.reduce((groups, item) => {
        const shop = String(item?.shopName || "未知店铺");
        groups.set(shop, (groups.get(shop) || 0) + 1);
        return groups;
      }, new Map())).map(([shop, count]) => `${shop} ${count}封`).join("；")
    : "";
  const confirmed = options.skipConfirm === true || await openActionConfirmModal(
    `确认执行所选 ${selections.length} 项${isShipping ? "单号回传" : "邮件发送"}任务吗？${shopSummary ? `店铺汇总：${shopSummary}。` : ""}该操作会${isShipping ? "实际调用乐天接口回传快递单号" : "实际向买家发送邮件"}。`,
    selections.length === 1 ? "确认执行当前任务" : "确认批量执行",
    "确认执行",
  );
  if (!confirmed) return;
  const result = await withBusyButton(button, "执行中...", () => request(
    "/rakuten-rms-api/automation/manual-actions/execute",
    { method: "POST", body: JSON.stringify({ items: selections }) },
  ));
  const runs = Array.isArray(result?.results) ? result.results : [];
  const totals = runs.reduce((summary, run) => {
    summary.shippingSent += Number(run?.shipping?.sent || 0);
    summary.shippingSkipped += Number(run?.shipping?.skipped || 0);
    summary.shippingFailed += Number(run?.shipping?.failed || 0);
    summary.mailSent += Number(run?.mail?.sent || 0);
...
async function ignoreRakutenRmsPreviewConflicts(button) {
  const connectionId = String(getSelectedRakutenApiImportConnection()?.id || "").trim();
  const preview = state.rakutenApiImportPreview;
...
function renderMasterProductDetail(detail) {
  ...
  $("masterProductDetailSubtitle").textContent = detail?.product?.productId
    ...
    : "-";
  const backButton = $("masterProductBackToListBtn");
  if (backButton) {
    backButton.textContent = state.masterProductReturnPanel === "shoulderStrapProductManagement"
      ? "返回肩带产品管理"
      : "返回同步记录";
  }
...
function renderShoulderStrapMaterialTable(type, bodyId, summaryId) {
  const body = $(bodyId);
  const summary = $(summaryId);
  if (!body) return;
  const rows = (Array.isArray(state.shoulderStrapMaterials) ? state.shoulderStrapMaterials : [])
    .filter((item) => String(item?.productType || "").trim() === type);
  if (summary) summary.textContent = `共 ${rows.length} 个${type}产品`;
  body.innerHTML = rows.length
    ? rows.map((item) => `<tr>
        <td><strong>${escapeHtml(displayText(item?.productId))}</strong></td>
        <td>${escapeHtml(displayText(item?.productName))}</td>
        <td class="master-product-current-cell">${escapeHtml(displayText(item?.stockQty ?? 0))}</td>
        <td>${escapeHtml(formatDate(item?.updatedAt))}</td>
      </tr>`).join("")
    : `<tr><td colspan="4" class="muted">暂无${escapeHtml(type)}主产品</td></tr>`;
}

function renderShoulderStrapMaterialTables() {
  renderShoulderStrapMaterialTable("肩带本体", "shoulderStrapBodyMaterialBody", "shoulderStrapBodyMaterialSummary");
  renderShoulderStrapMaterialTable("肩带配件", "shoulderStrapAccessoryMaterialBody", "shoulderStrapAccessoryMaterialSummary");
}

async function loadShoulderStrapMaterials() {
  const result = await request("/master-products/shoulder-strap-materials");
  state.shoulderStrapMaterials = Array.isArray(result?.items) ? result.items : [];
  renderShoulderStrapMaterialTables();
  return state.shoulderStrapMaterials;
}
...
function renderShoulderStrapBomDraftItems() {
  ...
  container.innerHTML = items.length
    ? items.map((item, index) => {
        const options = ["肩带本体", "肩带配件"].map((type) => {
          const typeOptions = state.shoulderStrapMaterials
            .filter((material) => String(material?.productType || "").trim() === type)
            .map((material) => `<option value="${escapeHtml(String(material.productId))}" ${String(material.productId) === String(item.componentProductId) ? "selected" : ""}>${escapeHtml(`${material.productId} - ${material.productName}（库存 ${material.stockQty}）`)}</option>`)
            .join("");
          return `<optgroup label="${escapeHtml(type)}">${typeOptions}</optgroup>`;
        }).join("");
        return `<div class="master-product-bom-item shoulder-strap-bom-item" data-index="${index}">
          ...
          <label>BOM 材料
            <select class="shoulder-strap-bom-part-select" data-index="${index}" required>
              <option value="">请选择主产品</option>${options}
            </select>
          </label>
    ...
    : '<div class="master-product-bom-empty muted">尚未设置 BOM 材料。</div>';
  const addButton = $("addShoulderStrapBomItemBtn");
  if (addButton) addButton.disabled = items.length >= 10 || state.shoulderStrapMaterials.length === 0;
}

function addShoulderStrapBomDraftItem() {
  if (state.shoulderStrapBomDraftItems.length >= 10) {
    showToast("一个 BOM 最多添加 10 个材料", true);
    return;
  }
  if (!state.shoulderStrapMaterials.length) {
    showToast("请先在主产品管理中建立肩带本体或肩带配件", true);
    return;
  }
  state.shoulderStrapBomDraftItems.push({ componentProductId: "", quantity: 1 });
...
function renderShoulderStrapProductTable() {
  ...
  if (summary) {
    summary.textContent = `共 ${state.shoulderStrapProductsTotal || 0} 个肩带产品，已显示 ${rows.length} 个`;
  }
  body.innerHTML = rows.length
    ? rows.map((item) => {
        const productId = String(item?.productId || "").trim();
        ...
        return `<tr>
          ...
          <td>${escapeHtml(displayText(size))}</td>
          <td class="master-product-current-cell">${escapeHtml(displayText(item?.stockQty ?? 0))}</td>
          <td class="master-product-assemblable-cell">${assemblable}</td>
          <td>${renderShoulderStrapBomItems(item?.bomItems)}</td>
          <td><button type="button" class="tiny-btn" data-action="manageShoulderStrapBom" data-product-id="${escapeHtml(productId)}">管理 BOM</button></td>
        </tr>`;
      }).join("")
...
async function loadShoulderStrapProducts({ reset = false } = {}) {
  ...
  const params = new URLSearchParams({
    page: String(page),
    pageSize: String(state.shoulderStrapProductsPageSize),
  });
...
function renderOverseasOrderProcessingTable() {
  ...
  if (!list.length) {
    tbody.innerHTML = '<tr><td colspan="13" class="muted">暂无可归结的海外仓订单</td></tr>';
  ...
  tbody.innerHTML = list
    .map(
      ...
      <tr>
        ...
        <td>${escapeHtml(getOverseasPickingBatchStatusText(item))}</td>
...
function renderYamatoShipmentBatchControls() {
  ...
  const completeBatchWorkBtn = $("overseasCompleteBatchWorkBtn");
...
function getIncompleteYamatoBatchWork(detail, batch) {
  const orders = Array.isArray(detail?.orders) ? detail.orders : [];
  const overseasOrders = orders.filter((item) => !isChinaDispatchModeValue(item.dispatchMode));
  return {
    pendingPageCount: getYamatoPendingPageCount(batch),
    waitingPrintOrderCount: overseasOrders.filter((item) => item.shipmentTrackingNo && !item.yamatoPrintedAt).length,
    pendingLabelOrderCount: overseasOrders.filter((item) => !item.shipmentTrackingNo).length,
  };
}

async function confirmOverseasBatchWorkComplete() {
  const detail = state.selectedOverseasPickingBatchDetail;
  if (!detail?.id) {
    throw new Error("请先选择一个拣货批次");
  }
  ...
  await refreshYamatoPrintStateForSelectedBatch();
  const refreshedDetail = state.selectedOverseasPickingBatchDetail;
  const batch = getSelectedYamatoShipmentBatch();
  if (!batch) {
    throw new Error("当前批次尚未生成 Yamato 面单批次，不能确认完成作业");
  }
  if (batch.status !== "pdf_ready") {
    throw new Error(`Yamato 批次 #${batch.id} 尚未上传面单 PDF，不能确认完成作业`);
  }

  const incomplete = getIncompleteYamatoBatchWork(refreshedDetail, batch);
  if (
    incomplete.pendingPageCount > 0 ||
    incomplete.waitingPrintOrderCount > 0 ||
    incomplete.pendingLabelOrderCount > 0
  ) {
    const details = [];
    if (incomplete.pendingPageCount > 0) {
      details.push(`还有 ${incomplete.pendingPageCount} 张面单未打印`);
    }
    if (incomplete.waitingPrintOrderCount > 0) {
      details.push(`涉及 ${incomplete.waitingPrintOrderCount} 条待打印订单`);
    }
    if (incomplete.pendingLabelOrderCount > 0) {
      details.push(`另有 ${incomplete.pendingLabelOrderCount} 条订单尚未生成或上传面单`);
    }
    throw new Error(`本批次作业未完成：${details.join("，")}。请完成后再次确认。`);
  }
...
function renderYamatoMergedScanSession() {
  ...
  const finalConfirmButton = $("confirmYamatoMergedScanBtn");
  if (!summary || !body) return;
...
function bindForms() {
  ...
  $("masterProductBackToListBtn").addEventListener("click", async () => {
    try {
      if (state.masterProductReturnPanel === "shoulderStrapProductManagement") {
  ...
  $("openMasterProductManagementPanel").addEventListener("click", async () => {
    try {
      state.masterProductReturnPanel = "syncRecords";
  ...
  $("openShoulderStrapProductManagementPanel")?.addEventListener("click", async () => {
    try {
      switchPanel("shoulderStrapProductManagement");
      await Promise.all([loadShoulderStrapProducts({ reset: true }), loadShoulderStrapMaterials()]);
    } catch (error) {
      showToast(error.message, true);
    }
  });
  $("refreshShoulderStrapProducts")?.addEventListener("click", async (event) => {
    try {
      await withBusyButton(event.currentTarget, "刷新中...", () =>
        Promise.all([loadShoulderStrapProducts({ reset: true }), loadShoulderStrapMaterials()]),
      );
    } catch (error) {
      showToast(error.message, true);
    }
  });
  $("shoulderStrapProductSearchForm")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    state.shoulderStrapProductKeyword = String($("shoulderStrapProductKeyword")?.value || "").trim();
    await loadShoulderStrapProducts({ reset: true });
  });
  $("resetShoulderStrapProductKeywordBtn")?.addEventListener("click", async () => {
  ...
  $("shoulderStrapBomItems")?.addEventListener("change", (event) => {
    ...
    if (!item) return;
    if (select) item.componentProductId = String(select.value || "");
  ...
  $("shoulderStrapBomForm")?.addEventListener("submit", async (event) => {
    ...
    const items = state.shoulderStrapBomDraftItems.map((item) => ({
      componentProductId: String(item.componentProductId || ""), quantity: Number(item.quantity),
    }));
  ...
  $("backToProductManagementFromShoulderStrap")?.addEventListener("click", async () => {
    try {
      await navigateToProductManagement();
    } catch (error) {
      showToast(error.message, true);
  ...
  $("yamatoMergedScanModal")?.addEventListener("click", (event) => {
    ...
    if (confirmPartButton) {
      confirmYamatoAssemblyPart(confirmPartButton.dataset.componentProductId)
        .catch((error) => showToast(error.message, true));
      return;
    }
  ...
  $("rakutenManualAutomationSelectAll")?.addEventListener("change", (event) => {
  ...
  $("rakutenManualAutomationBody")?.addEventListener("change", (event) => {
    const checkbox = event.target.closest("input[data-action='selectRakutenManualAutomation']");
    if (!checkbox) return;
    const key = String(checkbox.dataset.key || "");
    if (checkbox.checked) state.rakutenManualAutomationSelected.add(key);
    else state.rakutenManualAutomationSelected.delete(key);
    renderRakutenManualAutomation();
  });

  $("rakutenManualAutomationBody")?.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (action === "retryRakutenManualMail" || action === "markRakutenManualMailSent") {
        const label = action === "retryRakutenManualMail" ? "确认将该邮件恢复为待发送吗？" : "确认该邮件已经发送成功吗？";
    ...
    if (action !== "executeRakutenManualAutomation") return;
    ...
    if (!item) return;
    try {
      if (item.kind === "mail") await previewRakutenManualMailAction(item, button);
      else await executeRakutenManualAutomationItems([item], button);
    } catch (error) {
      showToast(error.message, true);
    }
  });

  $("applyRakutenManualMailFiltersBtn")?.addEventListener("click", async () => {
    state.rakutenManualMailPage = 1;
    try { await loadRakutenManualAutomationActions("mail"); } catch (error) { showToast(error.message, true); }
  });

  $("resetRakutenManualMailFiltersBtn")?.addEventListener("click", async () => {
    ["rakutenManualMailConnectionFilter", "rakutenManualMailOrderFilter", "rakutenManualMailFulfillmentFilter", "rakutenManualMailStatusFilter"]
      .forEach((id) => { const control = $(id); if (control) control.value = ""; });
    state.rakutenManualMailPage = 1;
    try { await loadRakutenManualAutomationActions("mail"); } catch (error) { showToast(error.message, true); }
  });

  $("prevRakutenManualMailPageBtn")?.addEventListener("click", async () => {
    if (state.rakutenManualMailPage <= 1) return;
    state.rakutenManualMailPage -= 1;
    try { await loadRakutenManualAutomationActions("mail"); } catch (error) { showToast(error.message, true); }
  });

  $("nextRakutenManualMailPageBtn")?.addEventListener("click", async () => {
    if (state.rakutenManualMailPage >= state.rakutenManualMailTotalPages) return;
    state.rakutenManualMailPage += 1;
    try { await loadRakutenManualAutomationActions("mail"); } catch (error) { showToast(error.message, true); }
  });

  $("confirmRakutenManualMailSendBtn")?.addEventListener("click", async (event) => {
  ...
  $("rakutenApiImportPreview")?.addEventListener("click", async (event) => {
    ...
    if (!button) return;
    try {
      await ignoreRakutenRmsPreviewConflicts(button);
    } catch (error) {
      showToast(error.message, true);
    }
  });
...
function bindDelegates() {
  ...
  $("resetRakutenAutomationRunFiltersBtn")?.addEventListener("click", async () => {
    if ($("rakutenAutomationRunConnectionFilter")) {
      $("rakutenAutomationRunConnectionFilter").value = String(state.rakutenMailScopedConnectionId || "");
    }
  ...
  $("resetRakutenMailFiltersBtn")?.addEventListener("click", async () => {
    ...
    if ($("rakutenMailConnectionFilter")) {
      $("rakutenMailConnectionFilter").value = String(state.rakutenMailScopedConnectionId || "");
    }
    try {
      await loadRakutenMails({ reset: true });
    } catch (error) {
  ...
  document.addEventListener("click", async (event) => {
    ...
    const rakutenManualAutomationClose = event.target.closest("button[data-action='closeRakutenManualAutomationModal']");
    if (rakutenManualAutomationClose) {
      closeModal("rakutenManualAutomationModal");
      return;
    }
    const rakutenManualMailPreviewClose = event.target.closest("button[data-action='closeRakutenManualMailPreviewModal']");
    if (rakutenManualMailPreviewClose) {
      state.rakutenManualMailPreviewItem = null;
      closeModal("rakutenManualMailPreviewModal");
      return;
    }
