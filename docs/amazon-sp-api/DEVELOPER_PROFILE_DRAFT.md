...
| 凭证是否安全存储，不在公共仓库共享或硬编码？ | **暂选否** | 生产密钥仅由 ECS `.env` 注入，JWT、MFA、Amazon 数据加密密钥、数据库应用凭证和 MySQL root 已随机生成；曾暴露的 XIYA/UOF 密钥完成轮换、旧密钥撤销及 Git 历史风险处置后改为“是”。 |
...
- [ ] ECS 已配置新的随机数据库、JWT、XIYA、UOF、Amazon LWA 和 Amazon 加密密钥；旧密钥全部撤销（数据库应用账号、MySQL root、JWT、MFA、Amazon 数据加密密钥已完成；XIYA/UOF 待办）；
