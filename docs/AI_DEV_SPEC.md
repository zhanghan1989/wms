...
- 手工确认批量入库（仅整单确认）
- 标准入库单流程仅用于批量导入场景
- 手工入库/调整（页面直接操作；支持通过 `SKU/rbSKU/ASIN/FNSKU` 任意一项检索产品，并可新增/调整货架、箱号、产品后执行库存增减）
...
### 8.2 基础资料
- `GET /api/skus`（支持按 `sku`、`rbSKU`、`ASIN`、`FNSKU` 任一字段检索）
- `POST /api/skus`
- `PUT /api/skus/:id`
- `GET /api/shelves`
- `POST /api/shelves`
...
- 手工检索：通过 `SKU/rbSKU/ASIN/FNSKU` 任一字段可检索到目标产品。
...
- 输入：
- 检索键（`SKU/rbSKU/ASIN/FNSKU` 任一项）与调整单明细（箱号、SKU、`qty_delta`、原因）。
- 处理：
